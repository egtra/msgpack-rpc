//
// msgpack::rpc::transport::tcp - MessagePack-RPC for C++
//
// Copyright (C) 2009-2010 FURUHASHI Sadayuki
//
//    Licensed under the Apache License, Version 2.0 (the "License");
//    you may not use this file except in compliance with the License.
//    You may obtain a copy of the License at
//
//        http://www.apache.org/licenses/LICENSE-2.0
//
//    Unless required by applicable law or agreed to in writing, software
//    distributed under the License is distributed on an "AS IS" BASIS,
//    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
//    See the License for the specific language governing permissions and
//    limitations under the License.
//
#include "../../types.h"
#include "base.h"
#include "tcp.h"
#include "cclog/cclog.h"
#include <mp/functional.h>
#include <mp/sync.h>
#include <mp/utilize.h>
#include <mp/object_delete.h>
#include <vector>
#include <iterator>

#include <winsock2.h>
#include <ws2tcpip.h>
#include <Windows.h>


namespace msgpack {
namespace rpc {
namespace transport {
namespace tcp {

namespace {

using namespace mp::placeholders;

class client_transport;
class server_transport;


class client_socket : public stream_handler<client_socket> {
public:
	client_socket(detail::unique_socket& sock, client_transport* tran, session_impl* s);
	~client_socket();

	void on_response(msgid_t msgid,
			object result, object error, auto_zone z);

private:
	client_transport* m_tran;
	weak_session m_session;

private:
	client_socket();
	client_socket(const client_socket&);
};


class client_transport : public rpc::client_transport {
public:
	client_transport(session_impl* s, const address& addr, const tcp_builder& b);
	~client_transport();

public:
	void send_data(sbuffer* sbuf);
	void send_data(auto_vreflife vbuf);

private:
	typedef std::vector<mp::shared_ptr<client_socket> > sockpool_t;
	typedef void (*finalize_t)(void* user);

	struct sync_t {
		sync_t() : sockpool_rr(0), connecting(0) { }
		sockpool_t sockpool;
		size_t sockpool_rr;
		unsigned int connecting;
		std::vector<WSABUF> buffer;
		std::vector<std::pair<finalize_t, void*> > finalizer;
	};

	typedef mp::sync<sync_t>::ref sync_ref;
	mp::sync<sync_t> m_sync;

	session_impl* m_session;

	double m_connect_timeout;
	unsigned int m_reconnect_limit;

private:
	void try_connect(sync_ref& lk_ref);
	static void on_connect(SOCKET sock, DWORD err, weak_session ws, client_transport* self);
	void on_connect_success(detail::unique_socket& sock, sync_ref& ref);
	void on_connect_failed(DWORD err, sync_ref& ref);

	struct call_finalizer;
	static void call_pending_xf_finalizer(const std::vector<std::pair<finalize_t, void*> >& finalizer);

	friend class client_socket;
	void on_close(client_socket* sock);

private:
	client_transport();
	client_transport(const client_transport&);
};


client_socket::client_socket(detail::unique_socket& sock, client_transport* tran, session_impl* s) :
	stream_handler<client_socket>(sock, s->get_loop_ref()),
	m_tran(tran), m_session(s->shared_from_this()) { }

client_socket::~client_socket()
{
	shared_session s = m_session.lock();
	if(!s) {
		return;
	}
	m_tran->on_close(this);
}

void client_socket::on_response(msgid_t msgid,
			object result, object error, auto_zone z)
{
	shared_session s = m_session.lock();
	if(!s) {
		throw closed_exception();
	}
	s->on_response(msgid, result, error, z);
}


client_transport::client_transport(session_impl* s, const address& addr, const tcp_builder& b) :
	m_session(s),
	m_connect_timeout(b.connect_timeout()),
	m_reconnect_limit(b.reconnect_limit()) { }

client_transport::~client_transport()
{
	// avoids dead lock
	//sockpool_t sockpool;

	//{
	//	sync_ref ref(m_sync);
	//	sockpool.swap(ref->sockpool);

	//	for(sockpool_t::iterator it(sockpool.begin()), it_end(sockpool.end());
	//			it != it_end; ++it) {
	//		(*it)->remove_handler();
	//	}
	//}
}

inline void client_transport::on_connect_success(detail::unique_socket& socket, sync_ref& ref)
{
	LOG_DEBUG("connect success to ",m_session->get_address()," fd=",socket.get());

	mp::shared_ptr<client_socket> cs = mp::make_shared<client_socket>(socket, this, m_session);

	ref->sockpool.push_back(cs);

	if(!ref->buffer.empty()) {
		std::vector<std::pair<finalize_t, void*> > finalizer;
		finalizer.swap(ref->finalizer); // finalizer = std::move(ref->finalizer);
		m_session->get_loop_ref()->send(cs->fd(), &ref->buffer[0], ref->buffer.size(),
			mp::bind(call_pending_xf_finalizer, finalizer));
		ref->buffer.clear();
	} else {
		call_pending_xf_finalizer(ref->finalizer);
		ref->finalizer.clear();
	}

	ref->connecting = 0;

	cs->receive_async();
}

void client_transport::on_connect_failed(DWORD err, sync_ref& ref)
{
	if(ref->connecting < m_reconnect_limit) {
		LOG_WARN("connect to ",m_session->get_address()," failed, retrying: ",mp::system_error::errno_string(err));
		try_connect(ref);
		++ref->connecting;
		return;
	}

	LOG_WARN("connect to ",m_session->get_address()," failed, abort: ",mp::system_error::errno_string(err));
	ref->connecting = 0;
	call_pending_xf_finalizer(ref->finalizer);

	ref.reset();
	m_session->on_connect_failed();
}

void client_transport::on_connect(SOCKET sock, DWORD err, weak_session ws, client_transport* self)
{
	detail::unique_socket socket(sock);
	shared_session s = ws.lock();
	if(!s) {
		return;
	}

	// FIXME
	int on = 1;
	//::setsockopt(socket.get(), IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&on), sizeof(on));

	sync_ref ref(self->m_sync);

	if(socket.get() != INVALID_SOCKET) {
		// success
		try {
			self->on_connect_success(socket, ref);
			return;
		} catch (...) {
			LOG_WARN("attach failed or send pending failed");
		}
	}

	self->on_connect_failed(err, ref);
}

struct client_transport::call_finalizer
{
	typedef void result_type;
	void operator()(const std::pair<finalize_t, void*>& pair) const
	{
		pair.first(pair.second);
	}
};

void client_transport::call_pending_xf_finalizer(const std::vector<std::pair<finalize_t, void*> >& finalizer)
{
	std::for_each(finalizer.begin(), finalizer.end(), call_finalizer());
}

void client_transport::try_connect(sync_ref& lk_ref)
{
	address addr = m_session->get_address();

	LOG_INFO("connecting to ",addr);

	union
	{
		sockaddr_storage addr_buf;
		sockaddr saddr;
	};
	addr.get_addr(&saddr);
	m_session->get_loop_ref()->connect(
		&saddr, addr.get_addrlen(), m_connect_timeout,
		mp::bind(
			&client_transport::on_connect,
			_1, _2,
			weak_session(m_session->shared_from_this()), this
			));
}

void client_transport::on_close(client_socket* sock)
{
	using namespace mp::placeholders;

	sync_ref ref(m_sync);
	sockpool_t::iterator found = std::find_if(
		ref->sockpool.begin(), ref->sockpool.end(),
		mp::bind(std::equal_to<client_socket*>(), mp::bind(&mp::shared_ptr<client_socket>::get, _1), sock));
	if(found != ref->sockpool.end()) {
		ref->sockpool.erase(found);
	}
}

void client_transport::send_data(sbuffer* sbuf)
{
	sync_ref ref(m_sync);
	if(ref->sockpool.empty()) {
		if(ref->connecting == 0) {
			try_connect(ref);
			ref->connecting = 1;
		}
		WSABUF buf = {sbuf->size(), sbuf->data()};
		ref->buffer.push_back(buf);
		ref->finalizer.push_back(std::make_pair(&::free, sbuf->data()));
		sbuf->release();
	} else {
		// FIXME pesudo connecting load balance
		mp::shared_ptr<client_socket>& sock = ref->sockpool[0];
		sock->send_data(sbuf);
	}
}

void client_transport::send_data(auto_vreflife vbuf)
{
	sync_ref ref(m_sync);
	if(ref->sockpool.empty()) {
		if(ref->connecting == 0) {
			try_connect(ref);
			ref->connecting = 1;
		}
		ref->buffer.reserve(ref->buffer.size() + vbuf->vector_size());
		std::transform(vbuf->vector(), vbuf->vector() + vbuf->vector_size(), std::back_inserter(ref->buffer), iovec_to_wsabuf());
		ref->finalizer.push_back(std::pair<finalize_t, void*>(&mp::object_delete<vreflife>, vbuf.get()));
		vbuf.release();
	} else {
		// FIXME pesudo connecting load balance
		mp::shared_ptr<client_socket>& sock = ref->sockpool[0];
		sock->send_data(vbuf);
	}
}


class server_socket : public stream_handler<server_socket> {
public:
	server_socket(detail::unique_socket& sock, shared_server svr); // detail::unique_socket&& sock
	~server_socket();

	void on_request(
			msgid_t msgid,
			object method, object params, auto_zone z);

	void on_notify(
			object method, object params, auto_zone z);

private:
	weak_server m_svr;

private:
	server_socket();
	server_socket(const server_socket&);
};


class server_transport : public rpc::server_transport {
public:
	server_transport(server_impl* svr, const address& addr);
	~server_transport();

	void close();

private:
	SOCKET m_lsock;
	loop m_loop;

	typedef mp::sync<std::vector<mp::shared_ptr<server_socket> > > sync_socket;
	typedef mp::weak_ptr<sync_socket> weak_sync_socket;
	mp::shared_ptr<sync_socket> m_socket;

	static void on_accept(SOCKET fd, DWORD err, weak_server wsvr, weak_sync_socket wsocket);

private:
	server_transport();
	server_transport(const server_transport&);
};


server_socket::server_socket(detail::unique_socket& sock, shared_server svr) :
	stream_handler<server_socket>(sock, svr->get_loop_ref()),
	m_svr(svr) { }

server_socket::~server_socket() { }

void server_socket::on_request(
		msgid_t msgid,
		object method, object params, auto_zone z)
{
	shared_server svr = m_svr.lock();
	if(!svr) {
		throw closed_exception();
	}
	svr->on_request(get_response_sender(), msgid, method, params, z);
}

void server_socket::on_notify(
		object method, object params, auto_zone z)
{
	shared_server svr = m_svr.lock();
	if(!svr) {
		throw closed_exception();
	}
	svr->on_notify(method, params, z);
}


server_transport::server_transport(server_impl* svr, const address& addr) :
	m_lsock(-1), m_loop(svr->get_loop_ref()), m_socket(mp::make_shared<sync_socket>())
{
	union
	{
		sockaddr_storage addr_buf;
		sockaddr saddr;
	};
	addr.get_addr(&saddr);

	m_lsock = m_loop->listen(
			PF_INET, SOCK_STREAM, 0,
			&saddr, addr.get_addrlen(),
			mp::bind(
				&server_transport::on_accept,
				_1, _2,
				weak_server(mp::static_pointer_cast<server_impl>(svr->shared_from_this())),
				weak_sync_socket(m_socket)
				));
}

server_transport::~server_transport()
{
	close();
}

void server_transport::close()
{
	if(m_lsock != INVALID_SOCKET) {
		::closesocket(m_lsock);
	}
	//if(m_lsock >= 0) {
	//	m_loop->remove_handler(m_lsock);
	//	m_lsock = -1;
	//}
}

void server_transport::on_accept(SOCKET socket, DWORD err, weak_server wsvr, weak_sync_socket wsocket)
{
	detail::unique_socket accept_socket(socket);
	shared_server svr = wsvr.lock();
	if(!svr) {
		return;
	}
	mp::shared_ptr<sync_socket> svr_socket = wsocket.lock();
	if (!svr_socket) {
		return;
	}

	if(accept_socket.get() == INVALID_SOCKET) {
		LOG_DEBUG("accept failed");
		return;
	}
	LOG_TRACE("accepted fd=",socket);

	// FIXME
	BOOL on = TRUE;
	::setsockopt(accept_socket.get(), IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&on), sizeof(on));

	sync_socket::ref ref(*svr_socket);
	ref->push_back(mp::make_shared<server_socket>(accept_socket, svr));

	ref->back()->receive_async();
}


}  // noname namespace

}  // namespace tcp
}  // namespace transport


tcp_builder::tcp_builder() :
	m_connect_timeout(10.0),  // FIXME default connect timeout
	m_reconnect_limit(3)      // FIXME default connect reconnect limit
{ }

tcp_builder::~tcp_builder() { }

std::auto_ptr<client_transport> tcp_builder::build(session_impl* s, const address& addr) const
{
	return std::auto_ptr<client_transport>(
			new transport::tcp::client_transport(s, addr, *this));
}


tcp_listener::tcp_listener(const std::string& host, uint16_t port) :
	m_addr(ip_address(host, port)) { }

tcp_listener::tcp_listener(const address& addr) :
	m_addr(addr) { }

tcp_listener::~tcp_listener() { }

std::auto_ptr<server_transport> tcp_listener::listen(server_impl* svr) const
{
	return std::auto_ptr<server_transport>(
			new transport::tcp::server_transport(svr, m_addr));
}


}  // namespace rpc
}  // namespace msgpack

